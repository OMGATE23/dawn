...
class DockerOrchestrator:
    def __init__(self, session_id: str, workspace_root: Path):
        ...
        self.project_dir = workspace_root / session_id
    ...
    def get_container(self):
        ...
        try:
            if self.client.containers.get(self.session_id): return self.restart_previous_container()
        except docker_errors.NotFound:
    ...
    def restart_previous_container(self):
        try:
            container = self.client.containers.get(container_id=self.session_id)
...
if __name__ == "__main__":
    input_session_id = input("Please give a run ID: ")
    workspace_root_path = Path(__file__).parent.resolve() / "workspace"
    orchestrator = DockerOrchestrator(
        session_id=input_session_id if input_session_id != "" else str(uuid.uuid4()),
